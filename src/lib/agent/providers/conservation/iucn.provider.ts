...
export class IUCNProvider implements ConservationProvider {
  ...
  async getConservationStatus(scientificName: string): Promise<ConservationResult> {
    ...
    if (!token) {
      logger.warn("IUCN API key missing. Using mocked IUCN provider.");
    ...
    try {
      ...
      const threatRes = await fetch(
        `${this.baseUrl}/threats/species/name/${encodeURIComponent(scientificName)}?token=${token}`,
      );
      ...
      const mainThreats =
        threatData.result
          ?.slice(0, 3)
          .map((t: any) => t.title)
          .join(", ") || "No major threats recorded";

      // 3. Fetch Habitats
      const habitatRes = await fetch(
        `${this.baseUrl}/habitats/species/name/${encodeURIComponent(scientificName)}?token=${token}`,
      );
