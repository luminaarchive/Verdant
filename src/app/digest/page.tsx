    ...
    <AppLayout>
      <div style={{ padding: '28px 32px 60px', maxWidth: '800px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
          <div>
            <h1 style={{ fontFamily: 'Georgia, serif', fontSize: '28px', fontWeight: '400', color: '#1A2F23', letterSpacing: '-0.5px', marginBottom: '4px' }}>Intelligence Digest</h1>
        ...
        {(summary.urgentCount > 0 || summary.importantCount > 0) && (
          <div className="card fade-up" style={{ padding: '16px 20px', marginBottom: '20px', borderLeft: `3px solid ${summary.urgentCount > 0 ? '#C0392B' : '#B8860B'}` }}>
        ...
        {/* Digest Entries */}
        {entries.length > 0 ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }} className="fade-up">
            {entries.map(entry => {
              const typeInfo = WATCHLIST_TYPES.find(t => t.id === entry.category)
              return (
                <div key={entry.id} className="card" style={{ padding: '16px 18px', cursor: 'pointer', transition: 'border-color 0.15s' }}
                ...
                >
                  <div style={{ display: 'flex', gap: '14px', alignItems: 'flex-start' }}>
                    <div style={{ flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
                      ...
                      <span style={{ fontFamily: "'Inter', sans-serif", fontSize: '9px', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.08em', color: SEV_COLORS[entry.severity] }}>{SEV_LABELS[entry.severity]}</span>
                    </div>
                    <div style={{ flex: 1 }}>
                      <p style={{ fontFamily: "'Inter', sans-serif", fontSize: '13.5px', fontWeight: '500', color: 'var(--text-main)', marginBottom: '4px', lineHeight: '1.4' }}>{entry.title}</p>
          ...
          <div className="card" style={{ padding: '48px 24px', textAlign: 'center' }}>
            ...
            <p style={{ fontFamily: 'Georgia, serif', fontSize: '18px', color: 'var(--text-main)', marginBottom: '8px' }}>Intelligence digest is clear</p>
        ...
        {summary.staleReports > 0 && (
          <div className="card fade-up" style={{ padding: '16px 18px', marginTop: '20px', borderLeft: '3px solid #B8860B' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
