...
import { addWatchlistItem } from '@/features/retention/watchlists'
import { recordActivity } from '@/features/retention/streak'
import { recordQuery, getRelatedPriorWork, getMemory } from '@/features/intelligence/memory'
import { computeReportScores, computeProofOfWork } from '@/features/intelligence/scores'
import { getReportFreshness, recordReportView, getReportDeltas } from '@/features/intelligence/living-reports'
import { getSourceReliability } from '@/features/intelligence/source-reliability'
...
function StructuredResult({ result, query, onRetry }: { result: ResearchResult; query: string; onRetry: () => void }) {
  ...
  const copyReport = () => {
    ...
    if (typeof result.executiveSummary === 'object' && result.executiveSummary) {
      ...
      if (result.executiveSummary.hiddenRisks) text += `Hidden Risks: ${result.executiveSummary.hiddenRisks}\n\n`
      if (result.executiveSummary.strategicImplications) text += `Strategic Implications: ${result.executiveSummary.strategicImplications}\n\n`
      if (result.executiveSummary.recommendedNextAction) text += `Recommended Action: ${result.executiveSummary.recommendedNextAction}\n\n`
      if (result.executiveSummary.whyThisMattersNow) text += `Why This Matters Now: ${result.executiveSummary.whyThisMattersNow}\n\n`
    ...
    if (result.decisionRecommendations && result.decisionRecommendations.length > 0) {
      ...
      result.decisionRecommendations.forEach((rec, i) => {
        text += `${i + 1}. ${rec.recommendation}\n`
        text += `   Rationale: ${rec.rationale}\n`
        if (rec.riskLevel) text += `   Risk: ${rec.riskLevel} | Urgency: ${rec.urgency || 'medium'}\n`
        text += `\n`
      })
    }
    ...
    if (result.uncertaintyNotes && result.uncertaintyNotes.length > 0) {
      text += `UNCERTAINTY & LIMITATIONS\n`
      result.uncertaintyNotes.forEach(note => {
        if (typeof note === 'object') {
          text += `\u2022 ${note.uncertainty} \u2014 ${note.reason}\n`
        } else {
          text += `\u2022 ${note}\n`
        }
      })
    ...
    if (result.sources && result.sources.length > 0) {
      ...
      result.sources.forEach((src, i) => {
        text += `[${i + 1}] ${src.title}${src.author ? ` \u2014 ${src.author}` : ''}${src.year ? ` (${src.year})` : ''}${src.url ? `\n     ${src.url}` : ''}\n`
      })
    }

    text += `\n---\nGenerated by Verdant AI \u2022 Confidence: ${result.confidenceScore ?? 'N/A'}/100\n`
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }} className="fade-up">
      ...
      <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', justifyContent: 'flex-end', marginBottom: '4px' }}>
        <button onClick={downloadPdf} disabled={exporting || !result.runId} className="btn btn-subtle" style={abStyle}><Download size={12} /> {exporting ? '...' : 'PDF'}</button>
