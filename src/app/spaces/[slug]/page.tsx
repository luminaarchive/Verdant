        ...
        <main style={{ flex: 1, overflowY: 'auto', padding: '32px' }}>
          <div style={{ maxWidth: '960px', margin: '0 auto' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '32px' }}>
              <div>
                <h1 style={{ fontFamily: 'Georgia, serif', fontSize: '32px', fontWeight: '400', color: '#1A2E1A', marginBottom: '6px' }}>{title}</h1>
                <p style={{ fontFamily: 'system-ui, sans-serif', fontSize: '14px', color: '#8A9288' }}>
                  A collection of your research threads.
                </p>
              </div>
              <div style={{ display: 'flex', gap: '12px' }}>
                <button style={{ background: '#F5F2EB', color: '#1A2E1A', border: '1px solid rgba(26,46,26,0.15)', borderRadius: '6px', height: '36px', padding: '0 16px', fontSize: '13px', fontFamily: 'system-ui, sans-serif', fontWeight: '500', cursor: 'pointer', transition: 'background 0.15s' }}
                  onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = '#EDE8DC'}
                  onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = '#F5F2EB'}
                >
                  Edit Space
                </button>
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '16px' }}>
              ...
              {[1, 2, 3].map((i) => (
                <Link key={i} href={`/research?q=${encodeURIComponent(`${title} research part ${i}`)}`} style={{ textDecoration: 'none' }}>
                  <div style={{ background: '#F5F2EB', border: '1px solid rgba(45,74,45,0.12)', borderRadius: '8px', padding: '20px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', transition: 'border-color 0.15s, box-shadow 0.15s' }}
                    onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = '#1A2E1A'; (e.currentTarget as HTMLElement).style.boxShadow = '0 2px 8px rgba(26,46,26,0.05)' }}
                  ...
                  >
                    <div>
                      <h3 style={{ fontFamily: 'Georgia, serif', fontSize: '16px', fontWeight: '400', color: '#1A2E1A', marginBottom: '4px' }}>{title} Exploration #{i}</h3>
